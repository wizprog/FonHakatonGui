...
public class Database {
	 ...
	 * resultSet.getString(2)); } return resultSet; } catch (Exception e) {
	...
	public void getLastHour(HashMap<String, int[]> hashmap) {
		...
		try (Statement statement = connection.createStatement()) {
			...
			statement.execute(String.format("CREATE VIEW UserByBs AS SELECT [\"naziv_bs\"] , [\"a_imei\"], AVG(CONVERT(INT,[\"trajanje\"])) AS 'SrednjaVrednost' FROM dbo.fon_hackaton_2018  WHERE [\"tip_zapisa\"] = '\"ODLAZNI POZIV\"' AND [\"vreme\"] BETWEEN '\"%s\"' AND '\"%s\"' GROUP BY [\"naziv_bs\"] , [\"a_imei\"]",from,to));
			statement.execute(String.format("DELETE FROM dbo.BS_CALLS"));
			statement.execute(String.format("INSERT INTO dbo.BS_CALLS SELECT [\"naziv_bs\"], AVG(SrednjaVrednost)  FROM dbo.UserByBs GROUP BY [\"naziv_bs\"] ORDER BY  AVG(SrednjaVrednost) DESC"));
			
			statement.execute(String.format("DROP VIEW dbo.UserByBs"));
			statement.execute(String.format("CREATE VIEW UserByBs AS SELECT [\"naziv_bs\"] , [\"a_imei\"], COUNT([\"trajanje\"]) AS 'SrednjaVrednostPoruka' FROM dbo.fon_hackaton_2018 WHERE [\"tip_zapisa\"] = '\"ODLAZNI SMS\"' AND [\"vreme\"] BETWEEN '\"%s\"' AND '\"%s\"' GROUP BY [\"naziv_bs\"] , [\"a_imei\"] " ,from,to));
			statement.execute(String.format("DELETE FROM dbo.BS_SMS"));
			statement.execute(String.format("INSERT INTO dbo.BS_SMS SELECT [\"naziv_bs\"], AVG(SrednjaVrednostPoruka)  FROM dbo.UserByBs GROUP BY [\"naziv_bs\"] ORDER BY  AVG(SrednjaVrednostPoruka) DESC"));
		    ...
		    statement.execute(String.format("INSERT INTO dbo.BS_INTERNET SELECT [\"naziv_bs\"], AVG(SrednjaVrednostInterneta) FROM dbo.UserByBs GROUP BY [\"naziv_bs\"] ORDER BY  AVG(SrednjaVrednostInterneta) DESC"));
		...
		try (Statement statement = connection.createStatement();
				ResultSet resultSetCalls = statement.executeQuery(String.format("SELECT dbo.UserByBs.[BS] AS 'BS' , dbo.UserByBs.[ACD] ,  dbo.UserByBs.[ASMS] , dbo.BS_INTERNET.[Average DownloadUpload] AS 'ADU'\r\n" + 
				...
				while(resultSetCalls.next()){
					int array[] = new int[3];
					if(resultSetCalls.getString(2)==null) array[0]=0;
					else array[0]=Integer.parseInt(resultSetCalls.getString(2));
					if(resultSetCalls.getString(3)==null) array[1]=0;
					else array[1]=Integer.parseInt(resultSetCalls.getString(3));
					if(resultSetCalls.getString(4)==null) array[2]=0;
					else array[2]=Integer.parseInt(resultSetCalls.getString(4));
					hashmap.put(resultSetCalls.getString(1),array);
				}
		System.out.println("YEYEYEYEYEYEYEY");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
